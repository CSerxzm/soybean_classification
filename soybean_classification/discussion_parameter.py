...
def discussion_rfc_depth(x_start,x_end):
    ...
    for depth in range(x_start,x_end):
        score=[]
        for i in range(0,10):
            rf = RandomForestClassifier(n_estimators=60, max_depth=depth)
            # 训练模式
            rf.fit(x_train, y_train)
            predictions = rf.predict(x_validation)
            accuracy_aux = accuracy_score(y_validation, predictions)
            score.append(accuracy_aux)
        all_mae_histories.append(np.mean(score))
    show_plt(x_start,x_end,all_mae_histories)

def discussion_rfc_treenumber(x_start,x_end):
    ...
    all_mae_histories = []
    for treenumber in range(x_start,x_end):
        score=[]
        for i in range(0,10):
            rf = RandomForestClassifier(n_estimators=treenumber, max_depth=10)
            # 训练模式
            rf.fit(x_train, y_train)
            predictions = rf.predict(x_validation)
            accuracy_aux = accuracy_score(y_validation, predictions)
    ...
    plt.xlabel("treenumber")
    plt.ylabel("accuracy")
    plt.title(u'子树数目讨论', FontProperties=font)
    plt.show()
...
#discussion_rfc_treenumber(1,100)      
